...
##############################################################################
# TO DO: Improve the process of selecting features to feed into a model.
...
# The matrix for the erros from each run
error_mat = np.zeros((runs + 1, 10), dtype=float)  # the last row averages over all the runs!
...
for k in range(n_folds):
    ...
    for run in range(0, runs):
        ...
        # model = Ridge(alpha = this_alpha)  # Uncomment for using ridge regression
    
        # Fit the model
        linreg = model.fit(X_train, y_train)
    
        #print('linear model parameters: {}'.format(linreg.get_params()))
