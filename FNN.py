...
for k in range(n_folds):
    ...
    for run in range(0, runs):
        ############### DEFINE A KERAS NEURAL NET  ##################
        
        #construct a new feed forward network object
        model = Sequential()
        
        # Input layer
        model.add(Dense(hidden_layer1_size, input_dim=input_layer_size, activation='relu',\
                        bias_regularizer=regularizers.l2(l2_reg)))
        
        # 2nd hidden layer
        if hidden_layers >=2:
            model.add(Dense(hidden_layer2_size, activation='relu', bias_regularizer=regularizers.l2(l2_reg)))
        
        # 3rd hidden layer
        if hidden_layers >=3:
            model.add(Dense(hidden_layer3_size, activation='relu', bias_regularizer=regularizers.l2(l2_reg)))
        ...
        # Early stopping: stops the training if after 20 epochs the error on the 
        # validation set has not changed for at least 1e-6            
        ...
        # history holds 'acc', 'loss', 'val_loss' - validation test loss function with epochs etc.
        
        # Evaluate the model
        scores = model.evaluate(X_train, y_train) # evaluating on training data
...
# save the forecasts
forecasts_all.to_csv("forecasts/forecasts_t" + str(k_hrs) + "_FNN_deep" + str(hidden_layers) + ".csv")